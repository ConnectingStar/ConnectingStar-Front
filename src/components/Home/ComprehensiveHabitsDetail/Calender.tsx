...
interface CalendarProps {
	setTargetDate: React.Dispatch<React.SetStateAction<TargetDate>>;
...
function Calender({ setTargetDate, targetDate, timeDiff }: CalendarProps): ReactNode {
	...
	useEffect(() => {
		...
		setDateList([...prev, targetDate, ...next]);
	}, []);
					...
					{dateList.map((el: DateListEl) => {
						...
						return (
							...
							>
								...
								<section
									css={css`
										${CalenderStyle.datePart}
									`}
								>
									<label
										css={css`
											${CalenderStyle.dateInner}
										`}
									>
										{/* 임시로 타겟을 설정 targetDate의 작동확인 */}
										{isTarget && "타겟"}
										{el.month}/{el.day}
									</label>
								</section>
